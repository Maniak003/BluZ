  ...
  while (1)
  {
    ...
    if ( connectFlag && (interval2 < intervalNow) /*&& system_startup_done*/) {
	  ...
	  if (dataType > onlyDozimeter) {							/* Нужно передавать спектр ? */
		...
		//	  tmpSpecterBuffer[jjj] = (uint32_t) jjj;
		  //}
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, A_SCK_Pin|A_CS_Pin|VIBRO_Pin|DISPLAY_CS_Pin
                          |DISPLAY_EN_Pin|LED_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : A_SCK_Pin A_CS_Pin VIBRO_Pin DISPLAY_CS_Pin
                           DISPLAY_EN_Pin LED_Pin */
  GPIO_InitStruct.Pin = A_SCK_Pin|A_CS_Pin|VIBRO_Pin|DISPLAY_CS_Pin
                          |DISPLAY_EN_Pin|LED_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
