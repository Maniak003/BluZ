 ...
 */
/**
 * Отрисовка гистограммы дозиметра — 512 значений (CPS за последние 512 секунд).
 *
 * Получает данные в [DeviceFrame.dosimeterData] на каждом BLE-фрейме. Текущее значение
 * слева — самое старое, справа — самое свежее. Содержит SMA-сглаживание (окно 20)
 * вторичным графиком.
 *
 * `init()` / `redrawDozimeter()` / `clearDozimeter()` — стандартный паттерн draw-классов
 * (см. [drawSpecter] для подробностей).
 */
class drawDozimeter {
    ...
    /** См. [drawSpecter.init]. Управляется флагом `GO.drawDozObjectInit`. */
    fun Init() {
    ...
    */
    /** Рисует две линии: основной сигнал (`dozPaint`, тонкая) и SMA-сглаживание окном 20
     *  (`dozPaintSMA`, толстая). Авто-масштабирование по [maxY] - [minY]. Если значения
     *  не меняются — линия по центру экрана. Слева — отступ [offsetX] для подписей. */
    fun redrawDozimeter() {
        ...
        textMax.text = "MAX ${maxY.toInt()}"
        textMin.text = "MIN ${minY.toInt()}"
    ...
    /** Заливает [dozCanvas] чёрным. Не сбрасывает [dozimeterData]. */
    fun clearDozimeter() {
