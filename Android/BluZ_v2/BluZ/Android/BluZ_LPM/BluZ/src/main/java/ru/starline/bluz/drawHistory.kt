...
/**
 * Отрисовка исторического спектра, сохранённого в памяти прибора. Аналогична [drawSpecter],
 * но без MLEM, zoom/pan и калибровки — просто визуализация.
 *
 * Данные приходят в [DeviceFrame.historyData] для frameType 4..6 (по команде `5u`).
 *
 * **Подводный камень.** [historyView] на странице spectrum может стартовать с `visibility="gone"`
 * (если пользователь не свайпал на историю) → `width=height=0` → bitmap не создаётся.
 * [redrawSpecter] имеет guard `!histCanvas.isInitialized || HSize <= 0 || VSize <= 0`,
 * чтобы не падать при вызове до layout.
 */
class drawHistory {
    ...
    /** См. [drawSpecter.init] — аналогично, но управляется флагом `GO.drawObjectInitHistory`. */
    fun init() {
    ...
    /** Отрисовка исторического спектра в [histCanvas]. Похожа на [drawSpecter.redrawSpecter],
     *  но без MLEM, zoom/pan и калибровки. Имеет guard на инициализацию [histCanvas] +
     *  ненулевые размеры — иначе тихо выходит. */
    fun redrawSpecter(spType: Int) {
        if (!this::histCanvas.isInitialized || HSize <= 0 || VSize <= 0) {
            Log.w("BluZ-BT", "history canvas not ready: HSize=$HSize VSize=$VSize")
            return
        }
    ...
    /** Заливает [histCanvas] чёрным. Не сбрасывает [historyData]. */
    fun clearHistory() {
