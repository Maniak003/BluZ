...
data = readmatrix('NaI_response_h1_Edep.csv', 'NumHeaderLines', 7);

% 2. Извлекаем ТОЛЬКО 1024 рабочих бина
%    Строка 1 = underflow, Строка 1026 = overflow
counts = data(2:end-1, 1);

% 3. Рассчитываем центры бинов для 1024 ячеек (диапазон 0–3 МэВ)
bin_centers = ((0:1023)' + 0.5) * 3.0 / 1024;
